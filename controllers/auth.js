...
exports.signup = async (req, res, next) => {
  try {
    ...
    if (existing_otp) await existing_otp.update({ otp: otp });
    else await Otp.create({ otp: otp, email: email });
...
exports.login = async (req, res, next) => {
  try {
    ...
    else if (shop) isValidShop = await bcrypt.compare(password, shop.password);
...
exports.googleSignup = async (req, res, next) => {
  try {
    ...
    const accesstoken = jwt.sign(
      { id: newUser.id, email: req.user.email },
      process.env.JWT_KEY_ACCESS,
      { expiresIn: "1h" }
    );
    const refreshtoken = jwt.sign(
...
exports.forgotPassVerify = async (req, res, next) => {
  try {
    ...
    const otpInDb = await Otp.findOne({
      where: { [Op.and]: [{ otp: otp }, { email: email }] },
    });
    if (!otpInDb) {
...
exports.newpass = async (req, res, next) => {
  try {
    ...
    const otpInDb = await Otp.findOne({
      where: {
        [Op.and]: [{ purpose: "forgot password verified" }, { email: email }],
      },
    });
...
exports.changePass = async (req, res, next) => {
  try {
    ...
    if (user) {
      if (user.googleId) {
        const err = new Error(
          "you cannot change password you can only reset it"
        );
        err.statusCode = 400;
        throw err;
      }
    ...
    if (isValidUser) newUser = user;
    else if (isValidShop) newUser = shop;
    if (newUser) {
      const hashedPw = await bcrypt.hash(newpass, 12);
