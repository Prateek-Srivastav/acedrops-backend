...
exports.createProduct = async (req, res, next) => {
  try {
    ...
    const prodCategory = await categories.findOne({
...
exports.home = async (req, res, next) => {
  try {
    ...
    const Shop = await shop.findAll({
      ...
      attributes: ["id", "shopName", "description"],
      limit: 4,
    ...
    const favProd = await fav.findAll({
...
exports.viewOneProd = async (req, res, next) => {
  try {
    ...
    if (prod) {
      ...
      const prodCat = await product_category.findOne({
        where: { productId: prodId },
      });
      const category = await categories.findOne({
        where: { id: prodCat.categoryId },
        attributes: ["category"],
      });

      reviewsAndRatings = await reviews.findAll({
        where: {
          [Op.and]: [
            { productId: prodId },
      ...
      userReview = await reviews.findOne({
        where: { [Op.and]: [{ productId: prodId }, { userId: req.user.id }] },
      });
      ...
      const favProd = await fav.findOne({
        where: { [Op.and]: [{ productId: prodId }, { userId: req.user.id }] },
        attributes: ["productId"],
      });
      if (favProd) isFav = true;
      return res
        .status(200)
        .json({ prod, category, userReview, reviewsAndRatings, rating, isFav });
    }
...
exports.addToCart = async (req, res, next) => {
  try {
    ...
    if (!cart) cart = await req.user.createCart({ price: 0 });
    const prodInCart = await cart.getProducts({ where: { id: prodId } });
    if (prodInCart.length > 0) prod = prodInCart[0];
...
exports.removeFromCart = async (req, res, next) => {
  try {
    ...
    if (prod) {
      ...
      if (newQuantity === 0) cart.removeProduct(prod);
      //update new quantity and price
      else await cart.addProduct(prod, { through: { quantity: newQuantity } });
      return res
        .status(200)
        .json({ price: cart.price, quantity: newQuantity, prodId: prod.id });
    } else {
      const err = new Error("no products to remove");
      err.statusCode = 400;
      throw err;
...
exports.viewCart = async (req, res, next) => {
  try {
    ...
    const prodInCart = await cart.getProducts({
      include: [
        { model: imgUrl, attributes: ["imageUrl"] },
...
exports.addAndRemFav = async (req, res, next) => {
  try {
    ...
    if (prod) {
      ...
      favInDb = await fav.findOne({
        where: { userId: req.user.id, productId: req.body.prodId },
      });
      if (favInDb) {
...
exports.viewWishlist = async (req, res, next) => {
  try {
    ...
    const result = await fav.findAll({
      where: { userId: req.user.id },
      attributes: ["productId"],
    });
