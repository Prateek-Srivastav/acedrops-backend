...
exports.search = async (req, res, next) => {
  try {
    ...
    const categoryProds = await categories.findAll({
      where: { category: { [Op.iLike]: `%${toSearch}%` } },
      include: [
        {
          model: product,
          include: [{ model: imgUrl, attributes: ["imageUrl"] }],
...
exports.rate = async (req, res, next) => {
  try {
    ...
    const alreadyReviewed = await reviews.findOne({
      where: { [Op.and]: [{ userId: req.user.id, productId: prodId }] },
    });
    ...
    if (orders.length > 0) {
      ...
      ordered = await order_item.findAll({
        where: {
          [Op.and]: [
            { orderId: orderIds },
            { productId: prodId },
      ...
      const err = new Error(
        "you can rate or review a product only after buying it"
      );
      err.statusCode = 400;
      throw err;
    }
...
exports.addAddress = async (req, res, next) => {
  try {
    ...
    await req.user.createAddress({
      houseNo: houseNo,
      streetOrPlotNo: streetOrPlotNo,
      locality: locality,
      city: city,
      state: state,
    });
        ...
        await userOrder.addProducts(
          products.map((prod) => {
            ...
            if (prod.stock < prod.cart_item.quantity) {
              let updatedPrice =
                prod.discountedPrice * (prod.cart_item.quantity - prod.stock);
              userOrder.increment({ price: -updatedPrice });
              prod.order_item = {
                quantity: prod.stock,
                customizationText: prod.cart_item.customizationText,
              };
            } else
              prod.order_item = {
                quantity: prod.cart_item.quantity,
                customizationText: prod.cart_item.customizationText,
              };
...
exports.orderProd = async (req, res, next) => {
  try {
    ...
    if (prod) {
      ...
      if (prod.stock < quantity) quantity = prod.stock;
...
exports.cancelOrder = async (req, res, next) => {
  try {
    ...
    } else return res.status(200).json("cannot cancel a delivered order");
  } catch (err) {
    if (!err.statusCode) err.statusCode = 500;
    next(err);
  }
